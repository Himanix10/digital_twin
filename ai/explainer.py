...
# ── Lazy load — model loads only when first called, not at import ──
_tokenizer = None
_model = None
...
def generate_explanation(prompt, max_new_tokens=120):
    tokenizer, model = get_model()  # ← loads model only here
    ...
    structured_prompt = (
        f"You are an industrial system monitoring AI.\n"
        f"Only discuss mechanical performance, anomalies, vibration, and system stability.\n\n"
        f"System Health: {health_value}%\n"
        f"Status: {status}\n"
        f"Anomalies: {anomalies}\n"
        f"Noise: {noise}\n"
        f"Risk Assessment: {risk_analysis}\n\n"
        f"Provide a detailed industrial explanation:"
    )
